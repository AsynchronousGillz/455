...
public final class DQ2 {
	...
	public DQ2() {
		this.F_MARRIED = this.M_MARRIED = this.TOTAL = Double.valueOf(0);
	}
	
	public void add_F_MARRIED(int _F_MARRIED) {
		this.F_MARRIED += _F_MARRIED;
		this.TOTAL += _F_MARRIED;
	}

	public void add_M_MARRIED(int _M_MARRIED) {
		this.M_MARRIED += _M_MARRIED;
		this.TOTAL += _M_MARRIED;
	}
	
	public String toString() {
		String ret = "";
		if (this.TOTAL != Double.valueOf(0)) {
			ret += new String(String.format("   %1$-20s %2$3.2f", "F NEVER MARRIED: ", (this.F_MARRIED/this.TOTAL)*100)+"%\n");
        	ret += new String(String.format("   %1$-20s %2$3.2f", "M NEVER MARRIED: ", (this.M_MARRIED/this.TOTAL)*100)+"%\n");
		} else { 	
