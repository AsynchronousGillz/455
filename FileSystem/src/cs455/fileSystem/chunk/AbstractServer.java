...
public abstract class AbstractServer extends Thread {
	...
	public AbstractServer(int port) {
		this.connectionGroup = new ThreadGroup("MessagingConnection threads") {
			public void uncaughtException(Thread thread, Throwable exception) {
				connectionException((Connection) thread, exception);
			}
		};
	 ...
	 * @param sPort
	 */
	final public void addConnection(String host, int sPort) {
		...
		Connection node = createConnection(clientSocket);
	 ...
	 * 
	 */
	synchronized final private Connection createConnection(Socket clientSocket) {
		try {
			clientSocket.setReceiveBufferSize(Integer.MAX_VALUE);
			return new Connection(this.connectionGroup, clientSocket, this);
		} catch (IOException e) {
	...
	final synchronized public void close() {
		...
		try {
			...
			for (Connection node : getNodeConnections()) {
				try {
					node.close();
				} catch (Exception ex) {
					ex.printStackTrace();
	...
	synchronized final public String[] getConnectionNames() {
		...
		Connection[] nodeThreadList = new Connection[size];
	 ...
	 * @return an array of Strings containing NodeConnection Names.
	 */
	final public Connection getConnection(String address) {
		int size = connectionGroup.activeCount();
		Connection[] nodeThreadList = new Connection[size];
		connectionGroup.enumerate(nodeThreadList);
		for (Connection node : nodeThreadList) {
	 ...
	 */
	final public Connection[] getNodeConnections() {
		Connection[] nodeThreadList = new Connection[connectionGroup.activeCount()];
		connectionGroup.enumerate(nodeThreadList);
	 ...
	 */
	protected abstract void nodeConnected(Connection client);
	 ...
	 */
	synchronized protected void nodeDisconnected(Connection node) {
	 ...
	 */
	final synchronized protected void connectionException(Connection client, Throwable exception) {
	...
	protected abstract void MessageFromNode(ProtocolMessage msg, Connection client);
